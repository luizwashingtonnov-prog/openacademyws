...
$db = get_db();
$assessmentType = COURSE_COMPLETION_ASSESSMENT;
$stmtCourses = $db->prepare('SELECT id, title FROM courses WHERE completion_type = ? ORDER BY title');
$stmtCourses->bind_param('s', $assessmentType);
$stmtCourses->execute();
$courses = $stmtCourses->get_result()->fetch_all(MYSQLI_ASSOC);

if (empty($courses)) {
    flash('Nenhum curso configurado com avaliacao final. Ajuste um curso para este modo antes de adicionar questoes.', 'info');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        $stmtCount = $db->prepare('SELECT COUNT(*) AS total FROM course_questions WHERE course_id = ?');
        $stmtCount->bind_param('i', $selectedCourseId);
        $stmtCount->execute();
        $questionCount = (int) ($stmtCount->get_result()->fetch_assoc()['total'] ?? 0);

        if ($questionCount >= QUESTION_COUNT) {
            flash('O curso ja possui o limite de ' . QUESTION_COUNT . ' questoes.', 'warning');
        } elseif (!$prompt || count($cleanOptions) < 2) {
            flash('Informe a pergunta e pelo menos duas alternativas.', 'warning');
            ...
            if (using_pgsql()) {
                $stmtQuestion = $db->prepare('INSERT INTO course_questions (course_id, prompt) VALUES (?, ?) RETURNING id');
            } else {
                $stmtQuestion = $db->prepare('INSERT INTO course_questions (course_id, prompt) VALUES (?, ?)');
            }
            $stmtQuestion->bind_param('is', $selectedCourseId, $prompt);
            if ($stmtQuestion->execute()) {
                $questionId = $stmtQuestion->insert_id;
                $stmtOption = $db->prepare('INSERT INTO question_options (question_id, option_text, is_correct) VALUES (?, ?, ?)');
                foreach ($optionsInput as $index => $text) {
                    $text = trim($text);
                    if ($text === '') {
                        continue;
                ...
                flash('Questao adicionada com sucesso.', 'success');
            } else {
    ...
    if ($action === 'delete') {
        ...
        $stmtDelete = $db->prepare('DELETE FROM course_questions WHERE id = ? AND course_id = ?');
        $stmtDelete->bind_param('ii', $questionId, $selectedCourseId);
    ...
    redirect('questions.php?course_id=' . $selectedCourseId);
}

$stmtQuestions = $db->prepare('SELECT id, prompt FROM course_questions WHERE course_id = ? ORDER BY id ASC');
$stmtQuestions->bind_param('i', $selectedCourseId);
...
<div class="mx-auto max-w-7xl space-y-8 px-4 pb-12">
    <div class="rounded-3xl border border-slate-200 bg-white/90 p-8 shadow shadow-slate-900/10">
        <h1 class="text-2xl font-bold tracking-tight text-brand-gray">Banco de questoes</h1>
        <p class="mt-2 text-sm text-slate-500">Monte avaliacoes consistentes e garanta o nivel minimo de aprovacao para seus cursos.</p>
    </div>

    <form class="rounded-3xl border border-slate-200 bg-white p-6 shadow shadow-slate-900/10" method="get">
        <label for="course_id" class="mb-2 block text-sm font-semibold text-slate-600">Selecione o curso</label>
        <div class="flex flex-col gap-3 sm:flex-row">
            <select class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm focus:border-brand-red focus:outline-none focus:ring-4 focus:ring-brand-red/20" id="course_id" name="course_id" onchange="this.form.submit()">
                <?php foreach ($courses as $course): ?>
                    <option value="<?php echo (int) $course['id']; ?>" <?php echo ((int) $course['id'] === $selectedCourseId) ? 'selected' : ''; ?>><?php echo htmlspecialchars($course['title']); ?></option>
                <?php endforeach; ?>
            </select>
            <a class="inline-flex items-center justify-center rounded-2xl border border-slate-300 px-4 py-3 text-sm font-semibold text-slate-600 transition hover:bg-slate-700 hover:text-white" href="course_detail.php?course_id=<?php echo $selectedCourseId; ?>">Gerenciar curso</a>
        </div>
    </form>
    ...
    <div class="grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(0,3fr)]">
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow shadow-slate-900/10">
            ...
            <p class="mt-1 text-sm text-slate-500">Total atual: <?php echo count($questions); ?>/<?php echo QUESTION_COUNT; ?></p>
        ...
        <div class="rounded-3xl border border-slate-200 bg-white shadow shadow-slate-900/10">
            <?php if (empty($questions)): ?>
                <div class="px-6 py-12 text-center text-sm text-slate-500">Nenhuma questao cadastrada para este curso. Comece adicionando o primeiro enunciado.</div>
            <?php else: ?>
