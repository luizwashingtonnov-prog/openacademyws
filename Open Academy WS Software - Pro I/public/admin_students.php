...
<div class="mx-auto max-w-7xl space-y-8 px-4 pb-12">
    <div class="rounded-3xl border border-slate-200 bg-white/90 p-8 shadow shadow-slate-900/10">
        ...
        <p class="mt-2 text-sm text-slate-500">Consulte as matr&iacute;culas, acompanhe o status de certificados e acesse as op&ccedil;&otilde;es de impress&atilde;o ou download.</p>
    </div>
                        ...
                        <?php else: ?>
                            <?php foreach ($enrolledCourses as $course): ?>
                                <?php
                                    ...
                                    $certificateGenerated = $certificateCode !== '';
                                    $issuedAtRaw = $course['issued_at'] ?? $course['created_at'] ?? null;
                                    $issuedAtDisplay = null;
                                    if ($issuedAtRaw !== null) {
                                        try {
                                    ...
                                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                        <div>
                                            ...
                                            <?php if ($course['score'] !== null): ?>
                                                <p class="mt-1 text-xs text-slate-500">Nota: <?php echo number_format((float) $course['score'], 1); ?> - <?php echo $approved ? 'Aprovado' : 'N&atilde;o aprovado'; ?></p>
                                            <?php else: ?>
                                                <p class="mt-1 text-xs text-slate-500">Avalia&ccedil;&atilde;o pendente</p>
                                            <?php endif; ?>
                                            <?php if ($certificateGenerated && $issuedAtDisplay !== null): ?>
